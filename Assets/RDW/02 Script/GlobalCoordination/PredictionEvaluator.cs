    ...
    public class PredictionEvaluator
    {
            ...
            string trajectoryMode)
        {
            ...
            for (int userId = 0; userId < physicalUsers.Count; userId++)
            {
                ...
                {
                    float horizon = Horizons[h];
                    if (horizon <= 0f)
                        continue;

                    Vector3 predictedPos = predictor.GetLinearPredictedPosition(userId, currentPos, horizon);
        ...
        public void ExportEpisodeSamplesCsv(string filePath)
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("EpisodeId,UserId,FrameIndex,SampleTime,Horizon,TrajectoryMode,PredX,PredY,PredZ,RealX,RealY,RealZ,Error");
            ...
            for (int i = 0; i < _episodeResolvedSamples.Count; i++)
            {
                ...
                sb.Append(s.Error.ToString(CultureInfo.InvariantCulture));
                sb.AppendLine();
            }
